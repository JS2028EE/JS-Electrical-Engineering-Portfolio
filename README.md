    - Engineering Portfolio - Jhostin Sanchez (2028) -
    




- Aspiring Electrical Engineer | PCB Designer | Problem-Solver

- Sophomore focused on electronics, physics, and hands-on engineering projects.









Technical Skills:

  - Electronics: KiCad, schematic design, PCB layout, soldering
  - Circuit Fundamentals: LEDs, resistors, voltage, power distribution
  - Microcontrollers: Arduino (beginner), basic embedded concepts
  - Software: Python
  - Tools: multimeter, continuity testing, breadboards
  - Engineering Strengths: debugging, iteration, self-directed learning










Current Goals: 





 - Build more advanced PCBs, including microcontroller-based boards
 - Develop stronger embedded programming skills
 - Expand project portfolio for admission to Columbia, MIT, and Georgia Tech
 - Learn deeper electronics fundamentals (grounding, noise, current flow)










 
Projects:







 ~ RGB LED PCB (Inolux IN-PS5TATRGB) ~





 - Type: Custom PCB

 - Software: KiCad

 - Skills: Routing, LED current calculations, resistor networks, schematic design, PCB layout, debugging





Overview:





Designed a custom PCB using four Inolux RGB LEDs and a resistor network. Focused on learning correct LED polarity, resistor placement, and power distribution.





What I Did





Created a full schematic in KiCad
Routed RGB channels using resistor networks
Corrected wiring mistakes after testing LED orientation
Practiced using proper ground and power routing
Learned how KiCad handles pads and footprints






What I Learned





Common-anode vs. common-cathode behavior
Proper resistor placement for RGB LEDs
How to connect multi-pin resistor packs correctly
Systematic debugging and schematic revision






Images





(Add schematic screenshots, PCB layout images, and any photos of prototypes.)









Basic LED and Switch Circuit (Intro PCB)





Type: Learning PCB

Skills: LED fundamentals, switches, simple routing, schematic creation





Overview





Designed a simple LED + push-button circuit to practice schematic design, routing, and understanding how current flows in a basic circuit.





What I Learned





Why LEDs require current-limiting resistors
How push-button switches work and how their pins map to footprints
How to route a basic single-layer board
How to identify and fix incorrect net connections




(Add screenshots of schematic and PCB.)









Coursework and Learning







Physics





Studied position, velocity, speed, and acceleration
Currently working on forces, energy, and Newton’s laws






Mathematics





Algebra II
Geometry
(Add future classes like Trigonometry, Precalculus once completed.)






Electronics (Self-Taught)





KiCad and PCB layout
Breadboard prototyping
Intro-level embedded systems










Certificates





(Add engineering-related certificates only: electronics courses, Python courses, Khan Academy physics mastery, etc.)









Engineering Notes / Build Logs





I keep notes on each project to show real engineering thinking:



Examples:



Incorrect LED polarity identified by tracing schematic → fixed orientation and routing.
Resistor network wired incorrectly → reviewed datasheet → rewired channels.
Switch footprint confusion → tested with continuity → updated schematic pins.




These logs demonstrate learning, iteration, and problem-solving.









Upcoming Projects





Microcontroller-controlled RGB PCB
5V regulator power supply board
IR or motion-based LED board
Custom Arduino-compatible PCB
Better revision of the RGB board with optimized routing










Contact





GitHub: (your username)

Email: (optional)

Location: United States
